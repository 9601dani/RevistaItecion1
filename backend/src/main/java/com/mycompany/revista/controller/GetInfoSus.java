 ...
 * @author daniel
 */
@WebServlet("/getInfoSuscripcion")
public class GetInfoSus extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        Suscripcion listA = new SuscripcionDaoImpl().getInfo(request.getParameter("usuario"), Integer.parseInt(request.getParameter("id")));
        SuscripcionConverter converter= new SuscripcionConverter(Suscripcion.class);
        if (listA != null) {
            response.getWriter().write(converter.toJson(listA));
        } else {
        }
    }
